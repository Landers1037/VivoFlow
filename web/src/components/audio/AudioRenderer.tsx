...
export function AudioRenderer(props: Props) {
  ...
  if (!isThreeAudioMode(props.mode) || webglFailed) {
    return <div className={cn("relative", props.className)}>
      <AudioCanvas {...props} mode={webglFailed ? "radial" : props.mode} className="h-full w-full" />
  ...
  return <div className={cn("relative overflow-hidden", props.className)}>
    <Suspense fallback={<AudioCanvas {...props} mode="radial" className="h-full w-full" />}>
      <ThreeAudioCanvas {...props} mode={props.mode} className="h-full w-full" onUnavailable={() => setWebglFailed(true)} />
    </Suspense>
  </div>;
}
