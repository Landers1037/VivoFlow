...
3. 顶栏：品牌名、连接状态、采集间隔、设置入口；`hide_title_bar` 开启时在仪表盘隐藏，右侧边缘唤出后临时覆盖显示。系统看板首页会在该全屏顶栏下方保留自己的单屏信息墙标题区。
4. 未连上且无快照时显示 `FullPageLoader`
5. 否则渲染 `Dashboard`；若开启系统看板，则优先渲染固定单屏 `SystemDashboard`，否则按时钟 / 黑洞 / 3D 模型 / 音乐 / 音频 / 相册 / 插画渲染对应全屏看板
6. 错误条展示服务端 / 解析错误
...
- 本地字体：[`web/src/fonts/`](../../web/src/fonts/) — `Outfit`（拉丁字母/数字，`unicode-range`）+ `Noto Sans SC`（中文）；时钟另用 Fusion Pixel、Barlow Condensed、DSEG7/14
